...
# AI Research Agent
# 
# --> 실시간 웹검색 API 없이도 동작하는 워크플로형 Agent입문 예제
#
# Agent단계 (순서대로 실행)
...
def build_report_prompt(topic: str, audience: str, 
                        goal: str, pasted_sources: str) -> tuple[str, str]:
    """
    최종 보고서 생성 프롬프트 함수
...
# =============== 버튼 클릭 처리 ============================
# try~except 예외처리 -> 오류가 나도 프로그램이비정상 종료되지 않는다.
try:
    # 1. 조사 계획 생성
    if plan_btn: # 버튼이 눌러졌다면
        with st.spinner('조사 계획 생성 중...'):
            system, user = build_plan_prompt(topic, audience, goal) # 계획 함수 호출
            # rufrhkfmf session_state에 저장 -> 재실행 후에도 결과 유지
            st.session_state.plan = call_llm(system, user) # llm함수 호출

    # 2. 자료 요약
    if summary_btn:
        # 자료없이 요약버튼을 누르면 경고만 표시하고 API는 호출하지 않는다.
...
# =============== 다운로드 버튼 표시 ===============
# 보고서, 발표 스크립트, Q&A중 하나라도 있으면 다운로드 버튼을 표시한다.
...
with st.expander('이게 왜 Agent인가요???'):
    st.markdown(
        '''
