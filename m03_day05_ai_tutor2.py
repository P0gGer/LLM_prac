...
def extract_pdf_pages(pdf_path: str) -> List[Dict[str, Any]]:
    ...
    반환 형식:
        [{"page":1, "text":"ㅁㄴㅇㄻㄴㅇㄹ..."}, {"page":2, "text":"ㄹㅇㅁㅇㄻㄴㄹㅇㅁㄴ..."},....]
    """
    doc = fitz.open(pdf_path)
    pages = [] # 결과
    for i, page in enumerate(doc, start=1): # 페이지번호를 1부터 시작
        text = page.get_text('text').strip() # .strip() : 양쪽 공백 제거
...
############################################
# 9. 이미지를 Base64 Data URL로 변환
############################################
def image_to_data_url(image_path: str) -> str:
    """
    이미지 파일을 Base64 형태로 변환해주는 함수
    Base64 Data URL 형태
        data:image/png;base46....
    """
...
def generate_summary_and_quiz(
    ...
    """
    자료 전체를 분석하여 요약, 핵심 키워드, 중요 페이지, 퀴즈를 JSON으로 반환한다.

    response_format에 json_schema를 지정하면 LLM이 반드시 해당 형식으로만 응답한다. -> 오류 없이 안정적으로 dict로 변환 가능
    """
    client = get_client()
...
다음 자료를 분석해서 요약, 핵심 키워드, 중요 페이지, 퀴즈 5개를 JSON으로 만들어라. 한국어로 작성하라.
...
if pdf_file:
        ...
        if st.button('요약 + 키워드 + 퀴즈 생성', type='primary'):
            try:
                with st.spinner('AI가 자료를 분석하는 중입니다...'):
                    result = generate_summary_and_quiz(pages, image_data_urls) # 함수 호출
    ...
    with tab2:
        question = st.text_input(
            '질문',
            placeholder='예: 이 자료의 핵심 개념을 초보자에게 설명해 줘.'
        )
        top_k = st.slider(
            '참고 페이지 수',
            min_value=1,
