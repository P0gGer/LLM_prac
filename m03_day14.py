...
OUTPUT_DIR.mkdir(exist_ok=True) # 폴더 만든다 - 이미 있으면 넘어간다
...
##################
# 4. 함수 정의 - 이미지를 부러와서 컴퓨터가 읽을 수 있도록 인코딩
...
##################
# 5. 함수 정의 - 영수증 분석
#   이미지를 GPT-4o-mini Vision으로 분석하고, JSON 형식에 맞춰 구조화된 데이터를 반환한다
#
##################
        ...
        messages=[
            {
                ...
                'content':[
                    ...
                    {
                        'type':'text',
                        'text':'이미지 속 영수증 또는 인보이스를 읽고 정확한 JSON으로 추출해 줘.'
                               '보이지 않는 값은 빈 문자열 또는 0으로 해 줘.'
                    },
                    # 이미지 (Base64 Data URL)
                    {
                        'type':'image_url',
...
##################
st.title(APP_TITLE)
st.caption('영수증/인보이스 이미지를 JSON과 CSV로 정리하는 멀티모달 고조화 출력 프로젝트입니다!')
...
if uploaded: # 업로드를 성공했다면(데이터가 있다면, 잘 불러와졌다면)
